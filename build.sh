...
diff ../m28c.config .config | grep -e "^<" | grep -v "^< #"
echo "diff initial config and new config (from new config only):"
diff ../m28c.config .config | grep -e "^>" | grep -v "^> #"
echo "check device exist"
grep -Fxq "CONFIG_TARGET_rockchip_armv8_DEVICE_widora_mangopi-m28c=y" .config || exit 1
...
#!/bin/sh
sed -i "1i ZZ_DISTRIB_NAME='ImmortalWrt for M28C'" /etc/openwrt_release
